...
import com.telecom.bean.PrintTemplateConfig;
...
public class TemplateConfigUtil {
	...
	private static final String TEMPLATE_CACHE_KEY_PREFIX = "DEMOPROJECT_TEMPLATE";// 缓存Key前缀
	...
	public static List<PageTemplateConfig> getAllPageTemplateConfigList() {
		...
		Element htmlConfigElement = (Element)document.selectSingleNode("/demoproject/page");
	...
	public static PageTemplateConfig getPageTemplateConfig(String name) {
		...
		try {
			...
			try {
				...
				Element element = (Element)document.selectSingleNode("/demoproject/page/" + name);
				String description = element.element("description").getTextTrim();
				String templatePath = element.element("templatePath").getTextTrim();
	 ...
	 * @return PageTemplateConfig对象
	 */
	public static PageTemplateConfig getWapPageTemplateConfig(String name) {
		PageTemplateConfig pageTemplateConfig = null;
		String cacheKey = TEMPLATE_CACHE_KEY_PREFIX + "_PAGE_" + name;
		GeneralCacheAdministrator generalCacheAdministrator = (GeneralCacheAdministrator) SpringUtil.getBean(CACHE_MANAGER_BEAN_NAME);
		try {
			...
			try {
				...
				pageTemplateConfig = new PageTemplateConfig();
				if (element.element("htmlPath") != null) {
					String wapHtmlPath = element.element("htmlPath").getTextTrim();
					pageTemplateConfig.setWapHtmlPath(wapHtmlPath);
				}
				pageTemplateConfig.setWapName(element.getName());
				pageTemplateConfig.setWapDescription(description);
				pageTemplateConfig.setWapTemplatePath(templatePath);
	 ...
	 * @return 模板文件内容
	 */
	public static String readMailTemplateFileContent(MailTemplateConfig mailTemplateConfig) {
	 ...
	 * 
	 */
	public static String writeMailTemplateFileContent(MailTemplateConfig mailTemplateConfig, String templateFileContent) {
		File templateFile = new File(CommonUtil.getWebRootPath() + mailTemplateConfig.getTemplatePath());
		try {
	...
	public static List<MailTemplateConfig> getAllMailTemplateConfigList() {
		...
		Element htmlConfigElement = (Element)document.selectSingleNode("/demoproject/mail");
		Iterator<Element> iterator = htmlConfigElement.elementIterator();
	...
	public static MailTemplateConfig getMailTemplateConfig(String name) {
		...
		try {
			...
			try {
				...
				Element element = (Element)document.selectSingleNode("/demoproject/mail/" + name);
				String description = element.element("description").getTextTrim();
				String subject = element.element("subject").getTextTrim();
				String templatePath = element.element("templatePath").getTextTrim();
	 ...
	 * 获取所有邮件模板配置集合
	 * 
	 * @return TemplateConfig集合
	 */
	@SuppressWarnings("unchecked")
	public static List<TemplateConfig> getAllTemplateConfigList() {
		File templateXmlFile = null;
		Document document = null;
		try {
			templateXmlFile = new File(CommonUtil.getWebRootPath() + TEMPLATE_XML_FILE_PATH);
		...
		Element htmlConfigElement = (Element)document.selectSingleNode("/demoproject/template");
		Iterator<Element> iterator = htmlConfigElement.elementIterator();
		List<TemplateConfig> allTemplateConfigList = new ArrayList<TemplateConfig>();
	    while(iterator.hasNext()) {
	    	Element element = (Element)iterator.next();
	    	String description = element.element("description").getTextTrim();
	    	String subject = element.element("subject").getTextTrim();
	    	...
	    	templateConfig.setName(element.getName());
	    	templateConfig.setDescription(description);
	    	templateConfig.setSubject(subject);
	    	templateConfig.setTemplatePath(templatePath);
	    	allTemplateConfigList.add(templateConfig);
	    }
		return allTemplateConfigList;
	}
	
	/**
	 * 根据名称获取TemplateConfig对象
	 * 
	 * @return TemplateConfig对象
	 */
	public static TemplateConfig getTemplateConfig(String name) {
		TemplateConfig templateConfig = null;
		String cacheKey = TEMPLATE_CACHE_KEY_PREFIX + "_" + name;
		GeneralCacheAdministrator generalCacheAdministrator = (GeneralCacheAdministrator) SpringUtil.getBean(CACHE_MANAGER_BEAN_NAME);
		try {
			templateConfig = (TemplateConfig) generalCacheAdministrator.getFromCache(cacheKey);
		} catch (NeedsRefreshException needsRefreshException) {
			boolean updateSucceeded = false;
			try {
				Document document = null;
				...
				Element element = (Element)document.selectSingleNode("/demoproject/template/" + name);
				String description = element.element("description").getTextTrim();
				String subject = element.element("subject").getTextTrim();
				String templatePath = element.element("templatePath").getTextTrim();
				...
				generalCacheAdministrator.putInCache(cacheKey, templateConfig);
				updateSucceeded = true;
			} finally {
	            if (!updateSucceeded) {
	            	generalCacheAdministrator.cancelUpdate(cacheKey);
	...
	/**
	 * 根据TemplateConfig对象读取模板文件内容
	 * 
	 * @return 模板文件内容
	 */
	public static String readTemplateFileContent(TemplateConfig templateConfig) {
		File templateFile = new File(CommonUtil.getWebRootPath() + templateConfig.getTemplatePath());
		String templateFileContent = null;
		try {
			templateFileContent = FileUtils.readFileToString(templateFile, "UTF-8");
		} catch (IOException e) {
	 ...
	 * 
	 */
	public static String writeTemplateFileContent(TemplateConfig templateConfig, String templateFileContent) {
		File templateFile = new File(CommonUtil.getWebRootPath() + templateConfig.getTemplatePath());
		try {
			FileUtils.writeStringToFile(templateFile, templateFileContent, "UTF-8");
		} catch (IOException e) {
	...
	public static List<PrintTemplateConfig> getAllPrintTemplateConfigList() {
		...
		Element htmlConfigElement = (Element)document.selectSingleNode("/demoproject/print");
		Iterator<Element> iterator = htmlConfigElement.elementIterator();
	...
	public static PrintTemplateConfig getPrintTemplateConfig(String name) {
		...
		Element element = (Element)document.selectSingleNode("/demoproject/print/" + name);
		String description = element.element("description").getTextTrim();
		String templatePath = element.element("templatePath").getTextTrim();
		PrintTemplateConfig printTemplateConfig = new PrintTemplateConfig();
		printTemplateConfig.setName(element.getName());
	...
	public static String processTemplateIntoString(String content,Map<String,String> params){
