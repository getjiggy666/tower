...
<body class="input admin">
	<div class="bar">
		<#if isAddAction>添加用户<#else>编辑用户</#if>
	</div>
		...
		<form id="validateForm" action="<#if isAddAction>admin!save.action<#else>admin!update.action</#if>" method="post">
			...
			<table class="inputTable tabContent">
				...
				<tr>
					...
					<td>
						<input type="text" name="admin.mobile" class="formText" value="${(admin.mobile)!}" />
						<label class="requireField">*</label>
					</td>
				</tr>
				<tr>
					...
					<td>
						<input type="password" name="admin.password" id="password" class="formText" title="密码长度只允许在4-20之间" autocomplete="off" />
					...
					<td>
						<select name="admin.department.id">
							...
							<#list departmentTreeList as departmentTree>
								<option value="${departmentTree.id}"<#if (departmentTree == admin.department)!> selected</#if>>
