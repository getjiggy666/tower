...
$().ready(function() {
	...
	var $smtpFromMailNick = $("#smtpFromMailNick");
	...
	$smtpTest.click( function() {
		...
		if ($this.val() == "邮箱测试") {
			...
			if ($.trim($smtpFromMailNick.val()) == "") {
				$.dialog({type: "warn", content: "请输入发件人昵称!", modal: true, autoCloseTime: 3000});
				return false;
			}
			...
			$.ajax({
				...
				data: { "setting.smtpFromMail": $smtpFromMail.val(),
						"setting.smtpHost": $smtpHost.val(),
						"setting.smtpPort": $smtpPort.val(),
						"setting.smtpUsername": $smtpUsername.val(),
						"setting.smtpPassword": $smtpPassword.val(),
						"setting.smtpFromMailNick": $smtpFromMailNick.val(),
						"smtpToMail": $smtpToMail.val() },
				type: "POST",
				dataType: "json",
				cache: false,
	...
	$validateForm.validate({
		...
		rules: {
			"setting.systemName": "required",
			"setting.systemUrl": "required",
		...
		messages: {
			"setting.systemName": "请填写系统名称",
			"setting.systemUrl": "请填写系统网址",
			"setting.loginFailureLockCount": {
		...
		<form id="validateForm" class="form" action="setting!update.action" enctype="multipart/form-data" method="post">
			...
			<table class="inputTable tabContent">
				<tr>
					...
					<td>
						<input type="text" name="setting.systemName" class="formText" value="${setting.systemName}" />
				...
				<tr>
					...
					<td>
						<input type="text" name="setting.systemUrl" class="formText" value="${setting.systemUrl}" />
						<label class="requireField">*</label>
					</td>
				</tr>
			</table>
			<table class="inputTable tabContent">
				<tr>
					...
					<td>
						<label>
							<@checkbox name="setting.isUpdating" value="${setting.isUpdating}" />启用
						</label>
					</td>
				</tr>
				<tr>
			...
			<table class="inputTable tabContent">
				...
				<tr>
					...
					<td>
						<input type="text" id="smtpFromMailNick" name="setting.smtpFromMailNick" class="formText" value="${setting.smtpFromMailNick}" />
						<label class="requireField">*</label>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						<input type="password" id="smtpPassword" name="setting.smtpPassword" class="formText" title="留空则不进行密码修改" autocomplete="off" />
					</td>
				</tr>
				<tr>
					<th>
